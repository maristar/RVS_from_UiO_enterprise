...
% Load the component's general interval around its individuaL peak
name_interval=['interval_' name_component];
try interval_temp=eval(name_interval);end



for mkk=1:(length(good_subj_list)) % Subject
   ...
   % For every condition
   peak_start_time_temp=peak_start_time(jjk);
   peak_end_time_temp=peak_end_time(jjk);
       ...
       if length(part_names_all)>0 % Parts --
            for gg=1:length(part_names_all)
                ...
                for cc=1:length(numchans); % Channels 
                    ...
                    [ final_peak_measure ] = RVS_Training_find_mean_ar_peak_measure_v2(temp_chan, peak_start_time_temp, peak_end_time_temp, Fs, timeVec_msec, type);
                ...
                elseif isempty(part_names_all);
                    ...
                    for cc=1:length(numchans); 
                        ...
                        %Define individual limits if no parts so general
                        %limits 
                        peak_start_time=interval_temp(mkk,1);
                        peak_end_time=interval_temp(mkk,2);
