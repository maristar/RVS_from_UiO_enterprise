...
bad_subject_list=[6,8,13,14,15,16,18,19,22,26,28]; % for Stim added 28 
...
for mkk=startfolder:length(good_subj_list)
    ...
    for mm=1:length(Sessions)
        ...
        for kk=1:length(conditions) % For every condition : Wrong, Correct,HR, LR
            ...
            Search_for_folder=['*_256__Luck_stim_' temp_condition_char '.set'];
...
% Make where to save
cd(Analyzed_path)
mkdir('FiguresGA_Training_Stim_noparts_accLuck')
cd FiguresGA_Training_Stim_noparts_accLuck
...
for cc=1:length(chanlocs)
    ...
    temp_save_name_fig=[chanlocs(cc).labels '_RVS_Training_Stim_accLuck_20_80'];
