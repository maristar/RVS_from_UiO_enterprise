...
startfolder=1;
for jjk=startfolder:length(temp22); % For every subject - folder
    ...
    for mm=1:length(Sessions)
        ...
        for kk=1:length(conditions) % For every condition : Wrong, Correct,HR, LR
            ...
            Search_for_folder=['*128_ch_DC_epochs_tr50_auto_5_chan_filt_FRN_triggers_' temp_condition_char '.set'];
...
% Make where to save
cd(Analyzed_path)
mkdir('FiguresGA_RVS_FRN_noparts')
cd FiguresGA_RVS_FRN_noparts
...
for cc=1:length(chanlocs)
    ...
    temp_save_name_fig=[chanlocs(cc).labels '_RVS_GA_FRN' 'Correct_Wrong'];
...
for cc=1:length(chanlocs)
    ...
    SP=0; line([SP SP], get(gca, 'ylim'), 'Color', [0 0 1]);
    text(0,max(GA_EEGdata_HR(cc,:)), 'Feedback');
    temp_save_name_fig=[chanlocs(cc).labels '_RVS_GA_FRN_HR_LR'];
