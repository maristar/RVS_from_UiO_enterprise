...
for kkj=1:length(sessions)
    ...
    [nchan ntimepoints ntrig]=size(data_all);
...
%% Forming the general name of the dataset 
   %pathname1='C:\Users\Maria\Desktop\1stdatas';
    ...
    %% TODO put here the saving the figures 
     ...
     for kk=1:nchan,
        ...
        title_name=[sessions{kkj} '-' names_chan{kk}];
        ...
        cd figures
        cd(stemp1)
        names_wave=[sessions{kkj} '_' names_chan{kk} '_wavelet']
  ...
  for kk=1:nchan,
        ...
        cd figures
        cd(stemp1)
        names_wave=[sessions{kkj} names_chan{kk} '_wavelet_norm_3'];
        saveas(H, names_wave, 'png')
        saveas(H, names_wave, 'fig')
        clear Btemp H names_wave
          end
